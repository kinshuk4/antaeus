...
class FirstOfMonthBillSchedulerTest {
    ...
    private var messageProducer = mockk<MessageProducer>() {
        every { sendMessage (any(), any<Int>(), any<String>()) } returns Unit
    }
    ...
    fun `will process pending invoices when there are some`() {
        ...
        dal = mockk {
            ...
            every { updateInvoiceStatusById(singleInvoice.id, InvoiceStatus.REQUESTED) } returns createInvoice(status = InvoiceStatus.REQUESTED)
        }
        invoiceService = InvoiceService(dal = dal)
        ...
        // Then
        assertTrue(output.trim().contains("Requesting the bills on 1st of the month"))
        assertTrue(output.trim().contains("Requesting invoice '$singleInvoice.id"))
    }
}